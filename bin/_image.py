...
# si la ayuda no es cargada se ejecuta el script
    ...
    if len(archivos) > 0:  # si existen archivos
        if consultArgument("fileformat", arg):
            tipo = consultParam("fileformat", arg)  # consulto el formato de imagen
        elif consultArgument("format", arg):
            tipo = consultParam("format", arg)
        else:
        ...
        if tipo in VALID_FILES:  # si el formato es correcto
            ...
            if len(archivos_validos) > 0:  # Si existen archivos tras el filtrado
                ...
                if consultArgument("size", arg):
                    ...
                    size = raw_input("Ingrese el tamano deseado [altoxancho]: ")
                if "x" in size:  # si el tamaño es valido
                    size = size.split("x")
                    if size[0].isdigit() and size[1].isdigit():  # si el tamano es valido
                        image_size[0] = int(size[0])
                        image_size[1] = int(size[1])
                        if image_size[0] > 0 and image_size[1] > 0:  # si son mayores a 0
                            if consultArgument("filename", arg):
                                namefile = consultParam("filename", arg)  # consulto el nombre a crear
                            else:
                                namefile = raw_input("Ingrese el nombre de archivo a crear: ")
                            if len(namefile) > 0:
                                if namefile == "%same%":
                                    keep = True
                                else:
                                    keep = False
                                if not keep:
                                    if consultArgument("count", arg):
                                        count = consultParam("count", arg)  # consulto el nombre a crear
                                    else:
                                        count = raw_input(
                                            "Ingrese el numero desde donde se comenzara a enumerar las imagenes: ")
                                else:
                                    count = "1"
                                if count.isdigit():  # si el contador es un numero
                                    number = int(count)
                                    # Cargo mas argumentos
                                    if consultArgument("lower", arg):
                                        lower = True
                                    else:
                                        lower = False
                                    if consultArgument("upper", arg):
                                        upper = True
                                    else:
                                        upper = False
                                    if consultArgument("silence", arg):
                                        silence = True
                                    else:
                                        silence = False
                                    for archivo in archivos_validos:  # Se recorren los archivos validos
                                        try:
                                            ...
                                            imagen.convert("RGBA")
                                            try:
                                                transfile = imagen.info["transparency"]
                                            except:
                                                pass
                                            ...
                                            if tipo == "gif":
                                                try:
                                                    ...
                                                    imagen.resize((image_size[0], image_size[1]),
                                                                  PIL.Image.ANTIALIAS).save(pngfilename,
                                                                                            transparency=transfile,
                                                                                            quality=100)
                                                except:
                                                    imagen.resize((image_size[0], image_size[1]),
                                                                  PIL.Image.ANTIALIAS).save(pngfilename, transparency=0,
                                                                                            quality=100)
                                            else:
                                                try:
                                                    imagen.resize((image_size[0], image_size[1]),
                                            ...
                                            try:
                                                transpng = imagen.info["transparency"]
                                            except:
                                                pass
                                            if keep:
                                            ...
                                            if newTipe == "gif":
                                                try:
                                                    imagen.save(ACTUAL_FOLDER + newimagefilename, transparency=transpng,
                                                                quality=100)
                                                except:
                                                    imagen.save(ACTUAL_FOLDER + newimagefilename, transparency=0,
                                                                quality=100)
                                            else:
                                                try:
                                                    imagen.save(ACTUAL_FOLDER + newimagefilename)
                                                except:
                                                    raise IOError
                                            del imagen
                                            ...
                                            print "Error :: Ocurrio un error al transformar la imagen '{0}'".format(
                                                archivo)
                                            print "Error :: Asegurese de que ningun programa este usando actualmente '{0}'".format(
                                                archivo)
                                        number += 1
                            else:
                                print "Error :: Nombre de archivo no valido"
                        else:
                            print "Error :: Tanto el ancho como el alto deben ser enteros mayores a cero"
                    else:
                        print "Error :: El tamano debe ser un digito"
                else:
                    print "Error :: El tamano no es válido"
            else:
                print "Error :: Ningun archivo tiene el formato pedido"
        else:
            print "Error :: Formato de archivo no valido"
    else:
        print "Error :: No hay archivos en el actual directorio"
