...
# -*- coding: utf-8 -*-
#Cursors - HOA
#Script creado para revisar los cursores disponibles
...
def loadCur(cursors,pos,app,pag="sig",curlabel=None,e=None): #Establece los cursores
    if pag == "sig": pos[0]+=1
    elif pag == "self": pass
    else: pos[0]-=1
    ...
    if not curlabel==None: curlabel.config(text="Cursor actual: "+cursors[pos[0]])

def copyToClipboard(cursors,pos,app,e=None): #Copia al clipboard
    app.clipboard_append(cursors[pos[0]])

root = Tk()
root.minsize(width=PROGRAM_SIZE[0],height=PROGRAM_SIZE[1])
root.resizable(width=False,height=False)
root.geometry('%dx%d+%d+%d' % (PROGRAM_SIZE[0],PROGRAM_SIZE[1],(root.winfo_screenwidth() - PROGRAM_SIZE[0]) / 2, (root.winfo_screenheight() -  PROGRAM_SIZE[1] -50) / 2))
root.title("Cursores")
frame = Frame(root)
...
root.iconbitmap("app.ico") #Icono del programa
root.mainloop(0)
