...
class board:
    ...
    def generatePlayers(self):
        ...
        self.players.append(group("PL", 1, self.player.getMaxLife() - self.player.getDamage(),
                                  self.player.getAttack() * COEF_PLAYER["ATK"] * (2 - self.dificultad_coef), \
            ...
            if followers > 0:
                self.players.append(
                    group("FL", followers, self.player.getMaxLife() * COEF_VIDA["L"] * self.dificultad_coef, \
                          ...
                          self.player.getDefensa() * COEF_DEF["L"] * self.dificultad_coef,
                          self.player.getLightImageFriend(), False, \
                          self.player.getTarget() * COEF_TAR["L"] * self.dificultad_coef,
            ...
            if followers > 0:
                self.players.append(
                    group("FM", followers, self.player.getMaxLife() * COEF_VIDA["M"] * self.dificultad_coef, \
                          ...
                          self.player.getDefensa() * COEF_DEF["M"] * self.dificultad_coef,
                          self.player.getMediumImageFriend(), False, \
                          self.player.getTarget() * COEF_TAR["M"] * self.dificultad_coef,
            ...
            if followers > 0:
                self.players.append(
                    group("FS", followers, self.player.getMaxLife() * COEF_VIDA["S"] * self.dificultad_coef, \
                          ...
                          self.player.getDefensa() * COEF_DEF["S"] * self.dificultad_coef,
                          self.player.getStrongImageFriend(), False, \
                          self.player.getTarget() * COEF_TAR["S"] * self.dificultad_coef,
    ...
    def generateMobs(self):
        ...
        while total_mobs > 0:  # Genero grupos de followers pesados
            ...
            self.mobs.append(group("MB", mobs,
                                   ...
                                   self.enemies.getDefensa() * COEF_MOB["DEF"] * (2 - self.dificultad_coef),
                                   self.enemies.getImage(), large, \
                                   self.enemies.getTarget() * COEF_MOB["TAR"] * (2 - self.dificultad_coef),
    ...
    def generateBoard(self):
        ...
        if BOARD_LIMIT[1] > (self.boardSize[1] + 1):
            self.boardCorrecion[0] = (BOARD_LIMIT[1] - self.boardSize[1] - 1) * 16 + 32  # Horizontal
        else:
            self.boardCorrecion[0] = 32
        if BOARD_LIMIT[0] > (self.boardSize[0] + 1):
            self.boardCorrecion[1] = (BOARD_LIMIT[0] - self.boardSize[0] - 1) * 16 + 32  # Vertical
        else:
        ...
        for i in range(self.boardSize[0] + 3):
            for k in range(0, last_pos + 1):  # Se colorea el bloque A
                try:
                    ...
                    light = str(sum(self.maplight[0]) / (self.boardSize[1] + 1))
    ...
    def placeFriends(self):
        ...
        while True:  # genero las posiciones
            ...
            if random_pos not in friends_pos and self.maplogics[random_pos[1]][
                random_pos[0]] == "none": friends_pos.append(random_pos)  # Si la posición random es válida
        ...
        while True:  # genero las posiciones
            if self.dificultad == "FACIL":
                ...
                random_pos = [random.randint(self.boardSize[1] - (1 + int(len(self.mobs) / 4)), self.boardSize[1]),
                              random.randint(0, self.boardSize[0])]
            if random_pos not in mobs_pos and self.maplogics[random_pos[1]][random_pos[0]] == "none": mobs_pos.append(
