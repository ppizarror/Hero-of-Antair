...
class mapeditor:
    ...
    def __init__(self):
        ...
        def _infoTile_edit(event=None):
            ...
            if self.isEvent:
                ...
                if main_logic == "3":  # Chest
                    ...
                    p.w.mainloop(1)
                    if p.sent: self.worldLogic[self.workTile[0]][
                    ...
                    if p.sent:
                        ...
                        properties = '8-0-' + data[0] + ',' + data[1] + ',' + tex + ',' + data[3] + ',' + data[4] + ',' \
                                     ...
                                     data[10] + ','
                        if data[11] != '':
                            properties += replaceStrict(data[11]) + ','  # Si el mob porta un objeto se agrega
                        else:
                            properties += '%NULL%,'
                    ...
                    q.w.mainloop(1)
                    if q.sent: self.worldLogic[self.workTile[0]][self.workTile[1]] = "13-0-" + replaceStrict(
                        q.values[0])
                    del q
                    ...
                    q.w.mainloop(1)
                    if q.sent: self.worldLogic[self.workTile[0]][self.workTile[1]] = "17-0-" + replaceStrict(
                        q.values[0])
                    del q
                elif main_logic == "18":  # Minusmana
                    q = pop(['Minus mana', self.images['minusmana_ico'], 'edit_absdigit', 77, 280, "Mana:", prop[0]])
                    q.w.mainloop(1)
                    if q.sent: self.worldLogic[self.workTile[0]][self.workTile[1]] = "18-0-" + replaceStrict(
                        q.values[0])
                    del q
                    ...
                    q.w.mainloop(1)
                    if q.sent: self.worldLogic[self.workTile[0]][self.workTile[1]] = "22-0-" + replaceStrict(
                        q.values[0])
                    del q
                elif main_logic == "23":  # Plusmana
                    q = pop(['Plus mana', self.images['plusmana_ico'], 'edit_absdigit', 77, 280, "Mana:", prop[0]])
                    q.w.mainloop(1)
                    if q.sent: self.worldLogic[self.workTile[0]][self.workTile[1]] = "23-0-" + replaceStrict(
                        q.values[0])
                    del q
                elif main_logic == "24":  # Teleport
                    q = pop(['Teleport', self.images['teleport_ico'], 'edit_maplink', 77, 280, prop[0]])
                    q.w.mainloop(1)
                    if q.sent: self.worldLogic[self.workTile[0]][self.workTile[1]] = "24-0-" + replaceStrict(
                        q.values[0])
                    del q
                    ...
                    q.w.mainloop(1)
                    if q.sent: self.worldLogic[self.workTile[0]][self.workTile[1]] = "28-0-" + replaceStrict(
                        q.values[0])
                    del q
                elif main_logic == "30":  # Longtext
                    q = pop(['Longtext', self.images['longtext_ico'], 'edit_longtext', 77, 280, putStrict(prop[0])])
                    q.w.mainloop(1)
                    if q.sent: self.worldLogic[self.workTile[0]][self.workTile[1]] = "30-0-" + replaceStrict(
                        q.values[0])
                    del q
                elif main_logic == "38":  # Sign
                    q = pop(['Sign', self.images['sign_ico'], 'edit_text', 75, 280, putStrict(prop[0])])
                    q.w.mainloop(1)
                    if q.sent: self.worldLogic[self.workTile[0]][
                    ...
                    p.w.mainloop(1)
                    if p.sent: self.worldLogic[self.workTile[0]][self.workTile[1]] = '42-' + sec_logic + '-' + p.values[
        ...
        def _ayuda(e=None):
            ...
            :return: void
            """
            pop([["Ayuda"], self.images["text_icon"], "license", 400, 600, DATA_DOCUMENTS + "/mapeditor/ayuda.txt"])
        ...
        def _licencia(e=None):
            ...
            :return: void
            """
            pop([["Licencia"], self.images["text_icon"], "license", 400, 600, "LICENSE"])
        ...
        self.infoNameMap = Label(l1, width=38, justify=LEFT, anchor=W, wraplength=380)
        self.infoNameMap.pack(anchor=NW)
        l5 = LabelFrame(self.menu1.interior, text="Autor del mapa", foreground=text_color_title, padx=3, pady=2)
        l5.pack(padx=0, pady=3)
        self.infoAutorMap = Label(l5, width=38, justify=LEFT, anchor=W, wraplength=380)
        self.infoAutorMap.pack(anchor=NW)
        l2 = LabelFrame(self.menu1.interior, text="Descripción", foreground=text_color_title, padx=3, pady=2)
        l2.pack(padx=0, pady=5)
        self.infoDescriptionMap = Label(l2, width=38, wraplength=270, justify=LEFT, anchor=W)
        self.infoDescriptionMap.pack(anchor=NW)
        l14 = LabelFrame(self.menu1.interior, text="Sonido de fondo", foreground=text_color_title, padx=3, pady=2)
        l14.pack(padx=0, pady=5)
        self.infoSoundMap = Label(l14, width=38, wraplength=270, justify=LEFT, anchor=W)
    ...
    def drawTiles(self):
        ...
        if self.showLines != False or self.showNumbers != False or self.showLogics != False:  # Dibujo la información de los tiles
            for i in range(0, self.mapSize[1]):
                if self.showLines: self.mapTile.create_line(32 * (i + 1) + 1, 0, 32 * (i + 1) + 1,
                                                            self.mapSize[1] * 32 + 2, fill=COLOR_LINE)
                k = -32
                for j in range(0, self.mapSize[0]):
                    k += 32
                    if self.showLines: self.mapTile.create_line(0, 32 * (i + 1) + 1, self.mapSize[0] * 32 + 2,
                    ...
                    if "actor" in self.worldItems[j][i]:  # Si se encuentra un actor (mob o npc)
                        ...
                        if actor[0] == "8":  # Si es un mob
                            ...
                            try:
                                ...
                                maxdist = 0
                            if velocidad > 0 and mov > 0 and maxdist != 0: self.setImageSquare(i, j, maxdist,
                                                                                               COLOR_MOVEMENT[0])
            ...
            if self.actualTexture != -1:  # Si lo seleccionado es una textura
                ...
                if t in TEXTURE_WATER or t in TEXTURE_LAVA and obj[
                    ...
                    if obj[0] == "4":  # Si el objeto previo era un tile de agua o liquido
                        ...
                        if prevTex in TEXTURE_LAVA: self.removeLightVector(tile[1],
                ...
                if self.worldLight[tile[0]][tile[1]] == 0:
                    ...
                    self.worldGraph[tile[0]][tile[1]] = self.actualTexture[1].replace("_0", "_1")
                    ...
                    if self.actualEnvironment[0] == "special":  # Si el item es algo especial, una llave o puerta, etc
                        ...
                        if tipo == "door" or tipo == "building":
                            ...
                            if self.worldLight[tile[0]][tile[1]] == 0:
                                ...
                                self.worldItems[tile[0]][tile[1]] = self.actualEnvironment[1].replace("_0", "_1")
                        elif tipo == "volcan":
                ...
                if tipo == "mob":
                    if self.worldLight[tile[0]][tile[1]] == 0:
                        ...
                        self.worldItems[tile[0]][tile[1]] = self.actualActor[0].replace("_0", "_1")
        ...
        if tipo == 1:  # Actors
            ...
            if prop == 1:  # Si el objeto es una puerta
                ...
                if tex in BUILD_LINKS_KEY:  # Link - door - key
                    ...
                    p.w.mainloop(1)
                    if p.sent: self.actualEnvironment = "special", tex, idItem + "-" + p.values[
            ...
            # TODO: Vehículo
                ...
                p.w.mainloop(1)
                if p.sent: self.actualEnvironment = "special", tex, '42-' + str(idItem) + '-' + p.values[
                    0] + ',' + replaceStrict(p.values[1]), "door"
                del p
            ...
            for j in range(-z + y, z + 1 + y):  # Se recorren los lados
                for i in range(-z + x, z + 1 + x):
                    if 0 <= j < self.mapSize[1]:  # Si la posicion de la matriz calza
                        if 0 <= i < self.mapSize[0]: self.mapTile.create_image(17 + 32 * i, 17 + 32 * j,
